...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the Telnyx library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Telnyx will fall back to '
                "an alternate HTTP library so everything should work. We "
                'recommend upgrading your "requests" library. If you have any '
                "questions, please contact support@telnyx.com. (HINT: running "
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the Telnyx library is falling back to urllib2/urllib "
...
class HTTPClient(object):
    ...
    def _max_network_retries(self):
        from telnyx import max_network_retries
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Telnyx\'s '
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = (
                "The Telnyx library attempted to fetch an "
                "invalid URL (%r). This is likely due to a bug "
                "in the Telnyx Python bindings. Please let us know "
                "at support@telnyx.com." % (url,)
            )
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from Telnyx."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = (
                "There was a problem receiving all of your data from "
                "Telnyx.  This is likely due to a bug in Telnyx. "
                "Please let us know at support@telnyx.com."
            )
        else:
            msg = (
                "Unexpected error communicating with Telnyx. If this "
                "problem persists, let us know at support@telnyx.com."
            )
...
class PycurlClient(HTTPClient):
    ...
    def request(self, method, url, headers, post_data=None):
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(pycurl.SSL_VERIFYHOST, True)
        else:
